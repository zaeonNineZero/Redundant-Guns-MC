 ...
 */
public class MaresLegRifleModel implements IOverrideModel
{
	private boolean hasExpanded = CGMExpandedHelper.isExpandedInstalled();
	private boolean disableAnimations = !hasExpanded;
	
	// The Mare's Leg Rifle is largely the same as the Lever-Action Rifle, only using a different body model.
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        BakedModel bakedModel = RedundantSpecialModels.MARES_LEG_RIFLE_BASE.getModel();
        if (getVariant(stack, "BaseVariant") == 1)
        bakedModel = RedundantSpecialModels.MARES_LEG_RIFLE_BASE_1.getModel();
